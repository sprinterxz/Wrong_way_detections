...
def detect(
        yolo_model=ROOT /'yolov5s.pt',
        deep_sort_model='osnet_x0_25',
        source=ROOT / 'videos/Traffic.mp4',
...
def draw_line_WW(im0,h,w):
    start_line_L = (0, h - 100)
    start_line_R = (w, h - 100)
    # print(start_line_R[])
    green = (0,255,0)
    red = (0,0,255)
    side = (220,255,0)
    cv2.line(im0, start_line_L, start_line_R, green, thickness=2)

    end_line_L = (270, h - 250)
    end_line_R = (w-270, h - 250)
    cv2.line(im0, end_line_L, end_line_R, red, thickness=2)
    arrow_start = (int(start_line_R[0]/2+start_line_L[0]/2),h - 100)
    arrow_end = (int(end_line_R[0]/2+end_line_L[0]/2),h - 250)
    cv2.arrowedLine(im0, arrow_start, arrow_end, (0,255,0), 2, tipLength=0.3)

    cv2.line(im0, start_line_L, end_line_L, side, thickness=2)
    cv2.line(im0, start_line_R, end_line_R, side, thickness=2)
