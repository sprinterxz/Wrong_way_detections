...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False, example='abc'):
        ...
        if self.pil:  # use PIL
            ...
            self.im = im
        self.lw = line_width or max(round(sum(im.shape) / 2 * 0.003), 1)  # line width
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            bbox_w = box[2] - box[0]
            bbox_h = box[3] - box[1]
            center_coordinates = (int(bbox_x + bbox_w / 2), int(bbox_y + (bbox_h / 2)))
            cv2.rectangle(self.im, p1, p2, color, thickness=2, lineType=cv2.LINE_AA)
            cv2.circle(self.im, center_coordinates, radius=3, color=color, thickness=2)
            if label:
